...
###################### Train predictin ##########################
    ...
    output_file_train = 'probabilities_train_epoch1.txt'

    # Write the predicted probabilities into the text file for the training set
    with open(output_file_train, 'w') as f:
        f.write("Predicted probabilities on the training set:\n")
        for i in range(train_probabilities.shape[0]):
            for j in range(train_probabilities.shape[1]):
                f.write(str(train_probabilities[i, j]) + ' ')
            f.write('\n')
     """
...
###################### Validation prediction ####################
    ...
    # write it into a txt file
    output_file = 'probabilities_promotor_epoch1000.txt'

# Écrire les probabilités prédites dans le fichier texte
    with open(output_file, 'w') as f:
      f.write("Probabilités prédites sur l'ensemble de validation :\n")
