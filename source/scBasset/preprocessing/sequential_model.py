...
def make_sequential_model(
    ...
    model.add(tf.keras.layers.Conv1D(filters=288, kernel_size=17,strides=1, padding= 'same', name='conv1d', use_bias=False))
    ...
    model.add(tf.keras.layers.Activation("relu"))
    model.add(tf.keras.layers.Dense(units =n_cells, activation='sigmoid', use_bias=True, name='dense_1'))
